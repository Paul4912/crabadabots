...
import { TeamData, TavernData, LootingData, MineData } from "./models/data";
import { getTeamsUrl, tavernUrlBigLimit, lootingUrl, getActiveGamesUrl, configs } from "./CrabadaApi";
...
async function main() {
  ...
  while (true) {
    try {
      ...
      await axios.get(getActiveGamesUrl(walletAddress), configs).then((response) => {
      ...
      for (let i = 0; i < activeMinesData.length; i++) {
        ...
        if (contract.shouldReinforce(mine, lastTimestamp)) {
          // Team requires reinforcing
          await axios.get(tavernUrlBigLimit, configs).then((response) => {
      ...
      for (let i = 0; i < teamData.length; i++) {
        ...
        if (team.status == "AVAILABLE") {
          // Team is doing jack shit, go loot some fuckers
...
function shouldLoot(ourTeam: TeamData, theirTeam: LootingData) {
  ...
  return isOurTeamStronger(ourTeam.battle_point, ourTeam.faction, theirTeam.defense_point, theirTeam.faction);
}
